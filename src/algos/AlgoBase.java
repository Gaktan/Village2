...
public class AlgoBase extends Algo {
	...
	public AlgoBase(boolean dude, Villager villager){
		health = (int) (Math.random() * (villager.getVillage().climat.difficulty * LIFE_EXPECTANCY));
		this.dude = dude;
		kids = 0;
						...
						if(babyCoolDown == 0){					//wants kids
							if(village.getPopulation() > 2){	//people in town
								...
								if(x > (100) - 25){						//25% chance making baby
									...
									AlgoBase algo = (AlgoBase) baby.algo;
									algo.health = 0;
